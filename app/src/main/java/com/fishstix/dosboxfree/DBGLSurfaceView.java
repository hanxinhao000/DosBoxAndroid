...
class DBGLSurfaceView extends GLSurfaceView implements SurfaceHolder.Callback {
	...
	@TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH)
	private class MySystemUiVisibilityChangeListener implements OnSystemUiVisibilityChangeListener {
	    @Override
	...
	float hatX = 0.0f;
	    MotionRange range = event.getDevice().getMotionRange(MotionEvent.AXIS_HAT_X, event.getSource());
		if (range != null) {
		    if (historyPos >= 0) {
	...
	public static class InputDeviceState {
       ...
       public static float ProcessAxis(MotionRange range, float axisvalue) {
